...
func TestBlockChain(t *testing.T) {
	...
	//        10
	testProcAddBlockMsg(t, mockDOM, blockchain)
	...
	testPrefixCount(t, mockDOM, blockchain)
	testAddrTxCount(t, mockDOM, blockchain)
	...
	testProcDelParaChainBlockMsg(t, mockDOM, blockchain)

	testProcAddParaChainBlockMsg(t, mockDOM, blockchain)
	testProcGetBlockBySeqMsg(t, mockDOM, blockchain)
	testProcBlockChainFork(t, blockchain)
	testDelBlock(t, blockchain)
	testIsRecordFaultErr(t)

	testGetsynBlkHeight(t, blockchain)
	testProcDelChainBlockMsg(t, mockDOM, blockchain)
	...
	// chunk
	testGetChunkRecordMsg(t, mockDOM, blockchain)
	testAddChunkRecordMsg(t, mockDOM, blockchain)
	testGetChunkBlockBodyMsg(t, mockDOM, blockchain)
	testAddChunkBlockMsg(t, mockDOM, blockchain)
}

func testGetChunkRecordMsg(t *testing.T, mockDOM *testnode.DplatformOSMock, blockchain *blockchain.BlockChain) {
	...
	msgGen := mockDOM.GetClient().NewMessage("blockchain", types.EventGetChunkRecord, records)
	err := mockDOM.GetClient().Send(msgGen, false)
	assert.NoError(t, err)
	chainlog.Debug("testGetChunkRecordMsg end --------------------")
}

func testAddChunkRecordMsg(t *testing.T, mockDOM *testnode.DplatformOSMock, chain *blockchain.BlockChain) {
	...
	msgGen := mockDOM.GetClient().NewMessage("blockchain", types.EventAddChunkRecord, records)
	err := mockDOM.GetClient().Send(msgGen, false)
	assert.Nil(t, err)
	chainlog.Debug("testAddChunkRecordMsg end --------------------")
}

func testGetChunkBlockBodyMsg(t *testing.T, mockDOM *testnode.DplatformOSMock, blockchain *blockchain.BlockChain) {
	...
	msgGen := mockDOM.GetClient().NewMessage("blockchain", types.EventGetChunkBlockBody, blocks)
	mockDOM.GetClient().Send(msgGen, true)
	resp, _ := mockDOM.GetClient().Wait(msgGen)
	if resp.GetData() != nil {
...
func testAddChunkBlockMsg(t *testing.T, mockDOM *testnode.DplatformOSMock, blockchain *blockchain.BlockChain) {
	...
	msgGen := mockDOM.GetClient().NewMessage("blockchain", types.EventAddChunkBlock, blocks)
	mockDOM.GetClient().Send(msgGen, false)
	chainlog.Debug("testAddChunkBlockMsg end --------------------")
}

func testProcAddBlockMsg(t *testing.T, mockDOM *testnode.DplatformOSMock, blockchain *blockchain.BlockChain) {
...
//            UpdateHeight2，     UpdateLastBlock2
//       GetBlockHeight     ， UpdateLastBlock2
// GetBlockHeight()        >= ProcGetLastHeaderMsg()
...
func testPrefixCount(t *testing.T, mockDOM *testnode.DplatformOSMock, blockchain *blockchain.BlockChain) {
	chainlog.Debug("testPrefixCount begin --------------------")

	msgGen := mockDOM.GetClient().NewMessage("blockchain", types.EventLocalPrefixCount, &types.ReqKey{Key: []byte("TxAddrHash:16ERTbYtKKQ64wMthAY9J4La4nAiidG45A:")})
	mockDOM.GetClient().Send(msgGen, true)
	Res, _ := mockDOM.GetClient().Wait(msgGen)
...
func testAddrTxCount(t *testing.T, mockDOM *testnode.DplatformOSMock, blockchain *blockchain.BlockChain) {
	chainlog.Debug("testAddrTxCount begin --------------------")
	cfg := mockDOM.GetClient().GetConfig()
	var reqkey types.ReqKey
	reqkey.Key = []byte(fmt.Sprintf("AddrTxsCount:%s", "16ERTbYtKKQ64wMthAY9J4La4nAiidG45A"))
	count, err := mockDOM.GetAPI().Query(cfg.ExecName("coins"), "GetAddrTxsCount", &reqkey)
	if err != nil {
		t.Error(err)
		return
	}
...
func testProcDelParaChainBlockMsg(t *testing.T, mockDOM *testnode.DplatformOSMock, blockchain *blockchain.BlockChain) {
	...
	msgGen := mockDOM.GetClient().NewMessage("blockchain", types.EventDelParaChainBlockDetail, &parablockDetail)
	mockDOM.GetClient().Send(msgGen, true)
	resp, _ := mockDOM.GetClient().Wait(msgGen)
...
func testProcAddParaChainBlockMsg(t *testing.T, mockDOM *testnode.DplatformOSMock, blockchain *blockchain.BlockChain) {
	...
	msgGen := mockDOM.GetClient().NewMessage("blockchain", types.EventAddParaChainBlockDetail, &parablockDetail)

	mockDOM.GetClient().Send(msgGen, true)
	_, err = mockDOM.GetClient().Wait(msgGen)
	if err != nil {
		t.Log(err)
...
func testProcGetBlockBySeqMsg(t *testing.T, mockDOM *testnode.DplatformOSMock, blockchain *blockchain.BlockChain) {
	...
	msgGen := mockDOM.GetClient().NewMessage("blockchain", types.EventGetBlockBySeq, &types.Int64{Data: seq})

	mockDOM.GetClient().Send(msgGen, true)
	msg, err := mockDOM.GetClient().Wait(msgGen)
	if err != nil {
		t.Log(err)
		//t.Error("testProcAddParaChainBlockMsg  only in parachain ")
	}
...
func testProcDelChainBlockMsg(t *testing.T, mockDOM *testnode.DplatformOSMock, blockchain *blockchain.BlockChain) {
...
func TestProcessDelBlock(t *testing.T) {
	...
	//       :    GetBlockHeight()        ，
	//      b.bestChain.Tip()            ,     ErrBlockHashNoMatch
...
func TestEnableCmpBestBlock(t *testing.T) {
	...
	mockDOM := testnode.NewWithConfig(defaultCfg, nil)
	...
	testCmpBestBlock(t, mockDOM.GetClient(), curblock.Block, cfg)
	...
	testCmpBestBlock(t, mockDOM.GetClient(), oldblock.Block, cfg)
