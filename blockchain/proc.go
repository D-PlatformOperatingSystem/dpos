...
//     block       ，     block parent block     best  tip
// blockchain      tx     ，
...
func (chain *BlockChain) addParaChainBlockDetail(msg *queue.Message) {
	...
	//    chain.cfgBatchSync parablockDetail.IsSync
	//              ,
...
//parachian   blockhash     seq，    addblock  seq
...
//GetLastBlockMainSequence      block
...
//GetMainSeqByHash parachian   blockhash     seq，    addblock  seq
...
//setValueByKey   kv  blockchain db
...
//GetValueByKey   value  key blockchain db
...
//getParaTxByTitleAndHeight //        +title
...
// getChunkBlockBody //   chunk BlockBody
...
func (chain *BlockChain) addChunkBlock(msg *queue.Message) {
	...
	if chain.GetDownloadSyncStatus() == chunkDownLoadMode {
		...
		for _, blk := range blocks.Items {
			_, err := chain.ProcAddBlockMsg(false, &types.BlockDetail{Block: blk}, "-self") //
			if err != nil {
