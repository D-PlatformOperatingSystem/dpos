...
const (
	// OnceMaxChunkNum         chunk
	OnceMaxChunkNum int32 = 30
	// DelRollbackChunkNum       chunk DelRollbackChunkNum
	DelRollbackChunkNum int32 = 10
	// MaxReqChunkRecord       MaxReqChunkRecord chunk record
...
func (chain *BlockChain) chunkProcessRoutine() {
	...
	checkGenChunkTicker := time.NewTicker(30 * time.Second) //         ，
	for {
		select {
		case <-chain.quit:
			return
...
// CheckDeleteBlockBody             BlockBody
	...
	for it.Rewind(); it.Valid(); it.Next() {
		...
		if data.Data < 0 {
			...
			if maxChunkNum > delChunkNum+int64(DelRollbackChunkNum) {
				kvs = append(kvs, &types.KeyValue{Key: key, Value: nil}) //     ToDeleteChunkSign
...
// GetChunkBlockBody  localdb    chunkbody
...
// CalcSafetyChunkInfo      chunkNum    chunk
...
// genChunkRecord        1:blockhash--->chunkhash 2:blockHeight--->chunkhash
