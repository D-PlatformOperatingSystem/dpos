...
func TestCreateGroupTxWithSize(t *testing.T) {
	...
	Decode(tx11, &tx12)
	tx12.Fee = 1
	//    size， fee=1,       size 998，     fee 100000，    tx[0].fee=200000，
...
func TestSortTxList(t *testing.T) {
	...
	var prevdomchainhashes [][]byte
	prevdomchainhashes = append(prevdomchainhashes, tx3211.Hash(), tx1114.Hash(), tx2214.Hash(), tx3214.Hash())
	...
	for j, sorttx := range sorTxList {
		if j < 9 {
			...
			assert.Equal(t, prevdomchainhashes[j-9], sorttx.Hash())
		} else if j >= 13 && j <= 16 {
