...
func TestConfigMverInit(t *testing.T) {
	...
	assert.Equal(t, cfg.MGStr("mver.consensus.name2", 0), "ticket-dom")
	assert.Equal(t, cfg.MGStr("mver.consensus.name2", 10), "ticket-domv5")
	assert.Equal(t, cfg.MGStr("mver.consensus.name2", 25), "ticket-domv2")
	...
	assert.Equal(t, cfg.MGStr("mver.exec.sub.coins.name2", -1), "ticket-dom")
	assert.Equal(t, cfg.MGStr("mver.exec.sub.coins.name2", 0), "ticket-domv5-enable")
	assert.Equal(t, cfg.MGStr("mver.exec.sub.coins.name2", 9), "ticket-domv5-enable")
	assert.Equal(t, cfg.MGStr("mver.exec.sub.coins.name2", 10), "ticket-domv5")
	assert.Equal(t, cfg.MGStr("mver.exec.sub.coins.name2", 11), "ticket-domv5")
...
func TestInitChainParam(t *testing.T) {
	...
	assert.Equal(t, cfg.GetP(forkid+1), getP(cfg, forkid+1))
	assert.Equal(t, cfg.GetFundAddr(), "1CQXE6TxaYCG5mADtWij4AxhZCUTpoABb3")
