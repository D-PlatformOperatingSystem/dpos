...
func TestGetAllCoinsMVCCIter(t *testing.T) {
	...
	m := getMVCCIter()
	defer m.db.Close()
	kvlist, err := m.AddMVCC(KeyValueList([2]string{"mavl-coins-dpos-exec-16ctbivFSGPosEWKRwxhSUt6gg9zhPu3jQ", "1"}, [2]string{"mavl-coins-dpos-16htvcBNSEA7fZhAdLJphDwQRQJaHpyHTq", "2"}), hashN(0), nil, 0)
	assert.Nil(t, err)
	saveKVList(m.db, kvlist)

	kvlist, err = m.AddMVCC(KeyValueList([2]string{"mavl-coins-dpos-16ctbivFSGPosEWKRwxhSUt6gg9zhPu3jQ", "3"}, [2]string{"mavl-coins-dpos-16htvcBNSEA7fZhAdLJphDwQRQJaHpyHTs", "4"}), hashN(1), hashN(0), 1)
	assert.Nil(t, err)
	saveKVList(m.db, kvlist)

	kvlist, err = m.AddMVCC(KeyValueList([2]string{"mavl-coins-dpos-exec-16ctbivFSGPosEWKRwxhSUt6gg9zhPu3jQ", "5"}, [2]string{"mavl-coins-dpos-16htvcBNSEA7fZhAdLJphDwQRQJaHpyHTt", "6"}), hashN(2), hashN(1), 2)
	assert.Nil(t, err)
	saveKVList(m.db, kvlist)
...
func TestSimpleMVCCLocalDB(t *testing.T) {
	...
	v, err := m.GetV([]byte("mavl-coins-dpos-exec-16ctbivFSGPosEWKRwxhSUt6gg9zhPu3jQ"), 0)
	assert.Nil(t, err)
	assert.Equal(t, "1", string(v))

	v, err = m.GetV([]byte("mavl-coins-dpos-exec-16ctbivFSGPosEWKRwxhSUt6gg9zhPu3jQ"), 1)
	assert.Nil(t, err)
	assert.Equal(t, "1", string(v))

	v, err = m.GetV([]byte("mavl-coins-dpos-exec-16ctbivFSGPosEWKRwxhSUt6gg9zhPu3jQ"), 2)
	assert.Nil(t, err)
	assert.Equal(t, "5", string(v))

	v, err = m.GetV([]byte("mavl-coins-dpos-exec-16ctbivFSGPosEWKRwxhSUt6gg9zhPu3jQ"), 3)
	assert.Nil(t, err)
	assert.Equal(t, "5", string(v))
