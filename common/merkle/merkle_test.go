...
//       roothash  branch.  bitcoin 99997 block    
// "height": 99997,
...
func TestCalcMainMerkleRoot(t *testing.T) {
	...
	//              
	tx1111, tx2211, tx3211 := modifyTxExec(tx12, tx22, tx32, "user.p.test.js", "user.p.para.lottery", "user.p.domchain.norm")
	...
	//  user.p.domchain.
	tx1114, tx2214, tx3214 := modifyTxExec(tx12, tx22, tx32, "user.p.domchain.norm", "user.p.domchain.coins", "user.p.domchain.retrieve")
	...
	// domchain     roothash[9-13]
	oldMixDomchainHash := calcSingleLayerMerkleRoot(sorTxList[9:13])
	mixChainHashes = append(mixChainHashes, oldMixDomchainHash)
	...
	//domchain    roothash
	assert.Equal(t, childMixChainHash[1].ChildHash, oldMixDomchainHash)
	assert.Equal(t, childMixChainHash[1].StartIndex, int32(9))
	assert.Equal(t, childMixChainHash[1].Title, "user.p.domchain.")
