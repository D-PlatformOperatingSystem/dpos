...
//
1. EventStoreSet(stateHash, (k1,v1),(k2,v2),(k3,v3)) ->    stateHash
...
func (store *BaseStore) processMessage(msg *queue.Message) {
	...
	} else if msg.Ty == types.EventStoreMemSet { //      set   ，
		store.wg.Add(1)
		go func() {
			defer store.wg.Done()
			datas := msg.GetData().(*types.StoreSetWithSync)
	...
	} else if msg.Ty == types.EventStoreCommit { //    set     commit
		store.wg.Add(1)
		go func() {
			defer store.wg.Done()
...
func (t *StorelistQuery) IterateCallBack(key, value []byte) bool {
	...
	if t.Mode == 2 { //prefix + suffix  ，   prefix     key  suffix   ，
