...
func TestIterateCallBack_Mode1(t *testing.T) {
	...
	execAddr1 := "16ctbivFSGPosEWKRwxhSUt6gg9zhPu3jQ"
	...
	execAddr2 := "26JvcBnNSE17fZhAd9JphBwQRQJaEpyHTp"
	datas = &types.StoreSet{
		StateHash: hash0,
	...
	execAddr3 := "36JvcBnNSE17fZhAd9JphBwQRQJaEpyHTp"
	datas = &types.StoreSet{
		StateHash: hash1,
...
func TestIterateCallBack_Mode2(t *testing.T) {
	...
	resp = &types.StoreListReply{}
	resp.End = []byte(addr)
	resp.Start = []byte("mavl-coins-dpos-exec-26JvcBnNSE17fZhAd9JphBwQRQJaEpyHTp:")
	resp.End = genPrefixEdge([]byte("mavl-coins-dpos-exec-26JvcBnNSE17fZhAd9JphBwQRQJaEpyHTp:"))
	resp.Count = 1
	resp.Mode = 2
	query = &drivers.StorelistQuery{StoreListReply: resp}
	store.IterateRangeByStateHash(hash2, resp.Start, resp.End, true, query.IterateCallBack)
...
//      Store.Get  ，         kvs(30 kv)，          ，  10000   ，    10000 。
...
//           ，            （hash  ），       MemSet Commit，              MemSet      Commit
