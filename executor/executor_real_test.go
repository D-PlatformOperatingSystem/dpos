...
func TestTxGroup(t *testing.T) {
	...
	mcfg := mockDOM.GetCfg()
	genkey := mockDOM.GetGenesisKey()
	mockDOM.WaitHeight(0)
	block := mockDOM.GetBlock(0)
	acc := mockDOM.GetAccount(block.StateHash, mcfg.Consensus.Genesis)
	...
	//
	block, err = util.ExecAndCheckBlock(mockDOM.GetClient(), block, txgroup.GetTxs(), []int{types.ExecOk, types.ExecOk, types.ExecOk})
	assert.Nil(t, err)
	assert.Equal(t, mockDOM.GetAccount(block.StateHash, mockDOM.GetGenesisAddress()).Balance, int64(9999999899700000))
	assert.Equal(t, mockDOM.GetAccount(block.StateHash, addr2).Balance, int64(0))
	assert.Equal(t, mockDOM.GetAccount(block.StateHash, addr3).Balance, int64(0))
	assert.Equal(t, mockDOM.GetAccount(block.StateHash, addr4).Balance, 1*types.Coin)
	...
	_, err = util.ExecAndCheckBlock(mockDOM.GetClient(), block, txgroup.GetTxs(), []int{types.ExecErr, types.ExecErr, types.ExecErr})
	assert.Nil(t, err)
	//      ：
	txs = nil
	txs = append(txs, util.CreateCoinsTx(cfg, genkey, addr2, types.Coin))
	...
	_, err = util.ExecAndCheckBlock(mockDOM.GetClient(), block, txgroup.GetTxs(), []int{types.ExecPack, types.ExecPack, types.ExecPack})
	assert.Nil(t, err)
	//      :
	txs = nil
	txs = append(txs, util.CreateCoinsTx(cfg, genkey, addr2, types.Coin))
	...
	_, err = util.ExecAndCheckBlock(mockDOM.GetClient(), block, txgroup.GetTxs(), []int{2, 2, 1})
	assert.Nil(t, err)
}
...
func TestExecAllow(t *testing.T) {
	...
	defer cfg.SetMinFee(prev)
	genkey := mockDOM.GetGenesisKey()
	genaddr := mockDOM.GetGenesisAddress()
	mockDOM.WaitHeight(0)
	block := mockDOM.GetBlock(0)
	...
	assert.Equal(t, mockDOM.GetAccount(block.StateHash, genaddr).Balance, 100000000*types.Coin)
	txs := []*types.Transaction{tx1, tx2, tx3, tx4}
	var err error
	block, err = util.ExecAndCheckBlockCB(mockDOM.GetClient(), block, txs, func(index int, receipt *types.ReceiptData) error {
...
func TestExecBlock2(t *testing.T) {
	...
	detail, _, err := util.ExecBlock(mockDOM.GetClient(), block.StateHash, block2, false, true, false)
	if err != nil {
		t.Error(err)
		return
	}
	...
	for i := 0; i < N; i++ {
		go func() {
			...
			detail, _, err := util.ExecBlock(mockDOM.GetClient(), block2.StateHash, block3, false, true, false)
			assert.Nil(t, err)
			for _, Receipt := range detail.Receipts {
				if Receipt.GetTy() != 2 {
					t.Errorf("exec expect true, but now false")
...
func TestSameTx(t *testing.T) {
	...
	newblock.Txs = util.GenNoneTxs(cfg, mockDOM.GetGenesisKey(), 3)
	...
	_, _, err := util.ExecBlock(mockDOM.GetClient(), nil, newblock, true, true, false)
	assert.Equal(t, types.ErrTxDup, err)

	//  2
	//[tx1,xt2,tx3,tx4,tx5,tx6] and [tx1,xt2,tx3,tx4,tx5,tx6,tx5,tx6]
	newblock.Txs = util.GenNoneTxs(cfg, mockDOM.GetGenesisKey(), 6)
	hash1 = merkle.CalcMerkleRoot(cfg, newblock.Height, newblock.Txs)
...
func TestExecBlock(t *testing.T) {
	...
	cfg := mockDOM.GetClient().GetConfig()
	mockDOM.WaitHeight(0)
	block0 := mockDOM.GetBlock(0)
	block := util.CreateCoinsBlock(cfg, mockDOM.GetGenesisKey(), 10)
	util.ExecBlock(mockDOM.GetClient(), block0.StateHash, block, false, true, false)
}
...
func BenchmarkExecBlock(b *testing.B) {
	...
	defer mockDOM.Close()
	cfg := mockDOM.GetClient().GetConfig()
	block := util.CreateCoinsBlock(cfg, mockDOM.GetGenesisKey(), 10000)
	mockDOM.WaitHeight(0)
	block0 := mockDOM.GetBlock(0)
	account := mockDOM.GetAccount(block0.StateHash, mockDOM.GetGenesisAddress())
