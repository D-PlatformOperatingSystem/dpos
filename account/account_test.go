...
func TestGetExecBalance(t *testing.T) {
	key := "mavl-coins-dpos-exec-16ctbivFSGPosEWKRwxhSUt6gg9zhPu3jQ:1OmCaA6un9CFYEWPnGRi7uyXY1KhTJxJEc"
	addr := "1OmCaA6un9CFYEWPnGRi7uyXY1KhTJxJEc"
	//prefix1 := "mavl-coins-dpos-exec-"
	//prefix2 := "mavl-coins-dpos-exec-16ctbivFSGPosEWKRwxhSUt6gg9zhPu3jQ:"
	...
	in.ExecAddr = []byte("16ctbivFSGPosEWKRwxhSUt6gg9zhPu3jQ")
	reply, err = getExecBalance(storeList3, in)
	assert.NotNil(t, err)

	fmt.Println("TestGetExecBalance---------------test case 4------------------------")
	in.ExecAddr = []byte("26JvcBnNSE17fZhAd9JphBwQRQJaEpyHTp")
	reply, err = getExecBalance(storeList3, in)
	assert.Nil(t, err)
	assert.Equal(t, int64(2), reply.Amount)
	assert.Equal(t, int64(1), reply.AmountFrozen)
...
func storeList1_1(req *types.StoreList) (reply *types.StoreListReply, err error) {
	...
	key1 := []byte("mavl-coins-dpos-exec-16ctbivFSGPosEWKRwxhSUt6gg9zhPu3jQ:1OmCaA6un9CFYEWPnGRi7uyXY1KhTJxJEc")
	key2 := []byte("mavl-coins-dpos-exec-26JvcBnNSE17fZhAd9JphBwQRQJaEpyHTp:1OmCaA6un9CFYEWPnGRi7uyXY1KhTJxJEc")
	key3 := []byte("mavl-coins-dpos-exec-36JvcBnNSE17fZhAd9JphBwQRQJaEpyHTp:1OmCaA6un9CFYEWPnGRi7uyXY1KhTJxJEc")
	reply.Num = 2
...
func getExecBalance(callback func(*types.StoreList) (*types.StoreListReply, error), in *types.ReqGetExecBalance) (reply *types.ReplyGetExecBalance, err error) {
	...
	req.End = in.Addr
	req.Mode = 2 //1： [start,end）  ，           。2： prefix + suffix    ，      ，           。
	req.Count = in.Count
	...
	for i := 0; i < len(res.Keys); i++ {
		...
		//  prefix  ：mavl-coins-dpos-exec-16ctbivFSGPosEWKRwxhSUt6gg9zhPu3jQ:  ,    addr           ，         。
...
func TestGetExecBalance2(t *testing.T) {
	...
	key := "mavl-coins-dpos-exec-16ctbivFSGPosEWKRwxhSUt6gg9zhPu3jQ:1OmCaA6un9CFYEWPnGRi7uyXY1KhTJxJEc"
	execAddr := "16ctbivFSGPosEWKRwxhSUt6gg9zhPu3jQ"
	addr := "1OmCaA6un9CFYEWPnGRi7uyXY1KhTJxJEc"
